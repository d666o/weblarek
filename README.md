...
### Данные
Описание интерфейсов для работы с данными

`IProduct` - интерфейс товара.
`IBuyer` - интерфейс покупателя.
`IValidate` - интерфейс валидации данных покупателя.
...
`IRejectOrder` - интерфейс для ответа Api после неудачной отправки данных.
...
`checkedItem: IProduct` - хранит товар, выбранный для подробного отображения.

Методы класса:
`setList(items: IProduct[]): void` - сохраняет массив товаров.
`getList(): IProduct[]` - возвращает массив товаров.
`getListItem(id: string): IProduct | undefined` - возвращает товар из массива по его id или `undefined`, если такого товара нет.
`setCheckedItem(item: IProduct | undefined): void` - сохраняет товар, выбранный для подробного отображения.
`getCheckedItem(): IProduct` - возвращает товар, выбранный для подробного отображения.
...
`removeItem(item: IProduct): void` - удаляет товар, который был получен в параметре, из массива корзины.
...
Методы класса:
`setInfo(item: IBuyer): void` - сохраняет данные покупателя.
`getInfo(): IBuyer` - возвращает все данные покупателя.
`clearInfo(): void` - очищает данные покупателя.
`validateInfo(): IValidate | true` - проверяет корректность внесенный данных. Возвращает объект с полями класса, значениями которых будет текст ошибки или `true`, если данные корректны.
...
`postOrder(item: IOrder): Promise<IResolveOrder | IRejectOrder>` - отправляет данные о покупке и возвращает ответ сервера.